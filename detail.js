...
// ========== 核心修复：GitHub Pages 仓库名前缀（关键！） ==========
const REPO_PREFIX = '/In-Memory-of-Medical-Students-Who-passed-Away'; // 仓库名，需和GitHub仓库名完全一致
...
function generateFormulaCaptcha() {
  ...
  // 渲染到你原有DOM（保留ID不变）
  const captchaFormulaEl = document.getElementById('captchaFormula');
  if (captchaFormulaEl) captchaFormulaEl.textContent = formulaText;
  // 清空输入和提示（内联script中生效的逻辑）
  const captchaInputEl = document.getElementById('captchaInput');
  const captchaTipEl = document.getElementById('captchaTip');
  if (captchaInputEl) captchaInputEl.value = '';
  if (captchaTipEl) captchaTipEl.style.display = 'none';
}
...
async function initPage(comradeId) { // 改为async
  ...
  const captchaFormulaEl = document.getElementById('captchaFormula');
  if (captchaFormulaEl) {
    captchaFormulaEl.addEventListener('click', generateFormulaCaptcha);
  }
  // 绑定提交按钮事件（内联script中生效的提交逻辑）
  bindSubmitEvent(comradeId);

  // 2. 核心修改：从comrades.json读取基础数据（添加仓库前缀）
  const comrade = await getComradeDetail(comradeId);
  if (comrade) {
    // 读取对应文件夹下的所有图片（传入仓库前缀）
    const photoPaths = await getFolderImages(comradeId, REPO_PREFIX);
    // 渲染个人信息（传入图片列表）
...
function bindSubmitEvent(comradeId) {
  ...
  submitBtn.addEventListener('click', () => {
    ...
    try {
      ...
      if (messageList) messageList.prepend(messageItem);
...
async function getComradeDetail(comradeId) {
  try {
    // 修复：添加仓库前缀，适配GitHub Pages路径
    const res = await fetch(`${REPO_PREFIX}/data/comrades.json`);
    if (!res.ok) throw new Error(`请求失败：${res.status}`);
    const data = await res.json();
    return data.find(item => item.id === comradeId);
  } catch (e) {
    console.error('加载逝者数据失败：', e);
    alert('加载逝者信息失败，请检查网络或刷新页面！');
    return null;
  }
}
...
function renderComradeDetail(comrade, photoPaths) {
  ...
  if (headerNameEl) headerNameEl.textContent = `${comrade.name || '未知'} 纪念页`;
        ...
        <div class="carousel-inner" style="transform: translateX(0);">
          ${photoPaths.map((path, index) => `
            <div class="carousel-item ${index === 0 ? 'active' : ''}">
              <img 
                src="${path}" 
                alt="${comrade.name || '逝者'}-${index+1}" 
                class="avatar gray-img" 
                data-src="${path}"
                onerror="this.src='${REPO_PREFIX}/icon/default-avatar.png'"
                loading="lazy"
              >
            </div>
          `).join('')}
...
function initCarousel(photoCount) {
  ...
  if (!carouselInner || !prevBtn || !nextBtn || !indicators.length) return;
...
function initImageModal() {
  ...
  if (!modalImg || !closeImgModal) return;
...
/**
 * 核心修复：渲染社交平台话题（添加仓库前缀，适配GitHub Pages）
 */
function renderSocial(data) {
  ...
  // 关键：平台与本地图标路径映射（添加仓库前缀！）
  const platformIconMap = {
    '知乎': `${REPO_PREFIX}/icon/zhihu.png`,
    '小红书': `${REPO_PREFIX}/icon/xhs.png`,
    '微博': `${REPO_PREFIX}/icon/weibo.jpg`,
    '微信公众号': `${REPO_PREFIX}/icon/weixin.png`,
    '搜狐': `${REPO_PREFIX}/icon/sh.jpg`,
    '未知平台': `${REPO_PREFIX}/icon/default.png` // 可选：新增默认图标
  };
      ...
      <div class="social-item">
        <div class="platform">
          ...
          <img 
            ...
            class="platform-icon" 
            alt="${platform}"
            onerror="this.src='${REPO_PREFIX}/icon/default.png'"
            loading="lazy"
          >
          ${platform}
        </div>
...
function saveMessage(comradeId, message) {
  ...
  try {
    ...
    console.error('留言存储失败（可能禁用了localStorage）：', e);
    alert('留言存储失败，请允许浏览器本地存储！');
  }
}
